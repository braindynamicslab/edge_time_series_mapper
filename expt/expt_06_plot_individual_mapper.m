...
for subject = subjects
    ...
    output_filename = sprintf("simplexMapper_%s_%d_%s_%s_peak_dense_pure_nodes_purity_%d_peak_density_%d.pdf", simplex, subject, session, parcellation, purity_threshold * 100, peak_density_threshold * 100);
    ...
    for colorbar_flag = [0, 1]
        fig = fcn_mapper_color_mapper_with_data(mapper_data, color_data, ...
            ...
            "max_color_data", max_color_data, ...
            "colorbar_flag", colorbar_flag);
        output_filename = sprintf("simplexMapper_%s_%d_%s_%s_centrality_colorbar_%d.pdf", simplex, subject, session, parcellation, colorbar_flag);
        exportgraphics(fig, fullfile(output_directory, output_filename), ...
            'ContentType','vector');
    end
