function fcn_peakDense_shuffle_task_labels(simplex, subject, session, parcellation, peak_threshold, peak_density_threshold, purity_threshold, num_shuffling, seed, shuffled_modularity_full_filename, shuffled_modularity_mean_filename)
...
filename = sprintf("simplex_mapper_%s_%d_%s_%s_%s_data.mat", ...
    simplex, subject, session, simplex, parcellation);
...
shuffled_mod = nan(num_shuffling, 3);
...
for shuffling_idx = 1:num_shuffling
    ...
    shuffled_mod(shuffling_idx, 2) = fcn_BCT_calMod(nodeBynode, shuffled_labels_peak_dense_pure_nodes);
    ...
    shuffled_mod(shuffling_idx, 3) = hoiAux_calMod(nodeBynode, shuffled_labels_matched_random_nodes);
end
...
full_table = array2table(shuffled_mod, ...
    'VariableNames', {'all', 'peak_dense_pure_nodes', 'matched_random_nodes'});
writetable(full_table, full_output_filename, 'VariableNamingRule', 'preserve');
