function [reduced_data, feature_indices, tasks_instantwise, feature_removal_mask, pca_results] = ...
    fcn_edgeMapper_get_processed_simplex_time_series_data(subject, parcellation, session, simplex, varargin)
    ...
    %   'debug_output_dir' - Debug output directory (default: <repo_root>/test/debug/test_<function_name>)
    %
    % Outputs:
    ...
    %   feature_removal_mask - [1 x num_ROIs] Logical mask of removed ROI features
    ...
    %
    % Example:
    %   % Basic usage - edge features, first 5min REST + tasks
    %   [data, idx, labels, mask, pca] = ...
    %       fcn_edgeMapper_get_processed_edge_time_series_data(100206, "schaefer100x7", "LR", "edge");
    %
    %   % Triangle features with PCA reduction
    %   [data, idx, labels, mask, pca] = ...
    %       fcn_edgeMapper_get_processed_edge_time_series_data(100206, "schaefer100x7", "LR", "triangle", ...
    ...
    parse(p, subject, parcellation, session, simplex, varargin{:});
    ...
    if debug_flag && strlength(debug_output_dir) == 0
        ...
        debug_output_dir = fullfile(config.repo_root, "test", "debug", ...
                                    "test_fcn_edgeMapper_get_processed_edge_time_series_data");
    ...
    valid_task_config = ["rest_5min_and_tasks", "rest_all_and_tasks", "tasks_only"];
    assert(ismember(task_configuration, valid_task_config), ...
        'task_configuration must be one of: %s\nGot: "%s"', ...
        strjoin(valid_task_config, ", "), task_configuration);
    ...
    [data_higher_features, feature_indices] = ...
        fcn_edgeMapper_generate_features(concatenated_data, simplex, ...
            'zscore_flag', 0, ...  % Already z-scored in preprocessing
